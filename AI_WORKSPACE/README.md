...
# ✅ **Summary for GitHub Copilot (drop this into your repo / README / prompt file)**

## **How this project uses GitHub Copilot**
This Home Assistant configuration uses GitHub Copilot for **authoring**, **refactoring**, and **maintaining** YAML, Jinja templates, packages, and diagnostic tools.  
Copilot is not used for runtime actions inside HAOS — only for code generation and structural improvements.

### **Copilot's responsibilities**
- Rewrite and clean YAML  
- Fix indentation and syntax errors  
- Convert old automations to new format  
- Generate Jinja templates  
- Build reusable packages  
- Create diagnostics helpers  
- Maintain modular folder structure  
- Assist with OneNote → SESSION_ESSENTIALS routing (markdown parsing, structuring, indexing)  
- Support audit‑driven documentation and commit messages  

### **Copilot is *not* responsible for**
- Executing MQTT actions  
- Diagnosing runtime entity states  
- Checking container uptime  
- Interacting with HAOS Developer Tools  
- Fixing live system issues  

### **Folder structure Copilot works with**
```
/config
  /automations
  /packages
...
### **Workflow**
1. Edit YAML/Jinja in VS Code with Copilot suggestions.  
2. Commit changes to Git (audit trail).  
3. Sync to HAOS via Git add‑on.  
4. Reload HA to apply changes.  

### **Copilot prompt style**
- Generate modular, audit‑safe YAML  
- Follow existing patterns in this repo  
- Avoid runtime assumptions  
- Provide comments and structure  
- Maintain readability and consistency  

---

If you want, I can also generate a **Copilot ruleset** or **.copilot/config.json** tuned specifically for your HA architecture.
